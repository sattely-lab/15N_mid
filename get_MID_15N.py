...
def calc_mz_windows(monoiso_mz, span, ppm, charge=1):
    """
    Finds a set of m/z ranges that contain 15N and 13C isotopologues of a given monoisotopic m/z.
    ...
    # loop through rows
    for m in row_idxs:
        mz_m = np.array((monoiso_mz + m*N15_NEUTRON, monoiso_mz + m*C13_NEUTRON))
...
def extract_mid_from_file(mzml_path, mz_windows, rt_window, ppm):
    ...
    with mzml.read(mzml_path) as reader:
        ...
        for m in m_span:
        	try:
        		mean_mz = np.average(mzs[m], weights=intensities[m])
        	except ZeroDivisionError:
        		mean_mz = 0
        	total_i = np.sum(intensities[m])
        	
        	mean_mz_vec.append(mean_mz)
        	total_i_vec.append(total_i)
            
        return({'m': np.asarray(m_span), 
               'mean_mz': np.asarray(mean_mz_vec), 
               'total_i': np.asarray(total_i_vec)})
...
def main():
    ...
    The dataframe has columns:
        ...
        n15_theo_mz,   float, m/z of ion with only m 15N atoms 
    ...
    N15_theo_mz = monoiso_mz + m*N15_NEUTRON
